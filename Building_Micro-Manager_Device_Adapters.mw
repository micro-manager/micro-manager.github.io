...
Before writing a new device adapter, you may find it worthwhile to examine the source code of some of the numerous open source device adapters already written, [https://valelab.ucsf.edu/svn/micromanager2/trunk/DeviceAdapters/ available in our subversion repository].
...
The CoreLog logging can contain helpful information (including whatever your device adapter logs using the <code>LogMessage()</code> function). Make sure to enable Debug Logging (in Tools > Options).

To debug DLL loading issues (such as when your device adapter is labeled "(unavailable)" in the Hardware Configuration Wizard), examine the CoreLog section from when you opened the Hardware Configuration Wizard. It should log the error causing the failure.
