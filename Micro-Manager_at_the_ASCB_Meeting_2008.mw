...
To demonstrate fast image acquistion we used a spinning disk confocal system ([http://www.yokogawa.com/scanner/products/csuX1e.htm Yokogawa]/[http://www.solameretech.com/ Solamere]) equipped with a [http://sales.hamamatsu.com/en/products/system-division/cameras/all-uv-vis-ir-cameras/part-c9100-13.php Hamamatsu EM camera (C9100-13)], [http://www.asiimaging.com/ ASI] XY and piezo X-stage, [http://www.zeiss.com Zeiss] AxioObserver Z1, multi-color laser launch and AOTF ([http://www.solameretech.com/ Solamere]), Mac Pro computer and screen ([http://www.apple.com Apple]) and an [http://www.arduino.cc/ Arduino] micro-controller. All equipment (except for the Arduino;) was made available to us by the respective company (Thanks!). For fast image acquistion, the camera was run in self-triggered mode, resulting in continuous acquisition at a rate limited almost only by the exposure time. Synchronization of the AOTF and piezo stage can not be accomplished by the computer since the computer has no way of figuring out when exactly the exposure happens. The solution is to use a TTL from the camera that signals when the camera is exposing (many, but not all scientific grade cameras have this capability) and use this TTL to drive a sequence of different output patterns on the Arduino micro-controller. Thus, the TTL of the camera is connected to an input port of the micro-controller. The micro-controller is then instructed to cycle through a sequence of digital output patterns. To this end, we wrote [https://valelab.ucsf.edu/svn/micromanager2/branches/micromanager1.3/DeviceAdapters/Arduino/AOTFcontroller/AOTFcontroller.pde a program] that runs on the Arduino and a Micro-Manager device adapter that interacts with this program (available in Micro-Manager version 1.3). A few other useful modes are included in this Arduino program, including a simple/cheap way to control TTLs (6 at the moment, but this can be expanded to 12), ability to do 'blanking' (only expose with light when the camera is exposing), and to run a sequence of events using the build-in clock of the Arduino. We also added a 2-channel DA chip so that we could control light intensity of the AOTF. Acquisition in Micro-Manager is run from a Beanshell [http://valelab.ucsf.edu/~nico/MMwiki/images/3/3f/MultiFastZStackASI.bsh script] that puts the incoming images in the correct order in the Image5D viewer
...
We were very encouraged by the interest and overwhelmingly positive response both from the user community and from the industry. On the ASCB exhibit floor we noted Micro-Manager controlling equipments in the booths of [http://www.point-source.com/newsview.asp?id=29 Point Source], [http://www.till-photonics.com/Products/imic.php Agilent], and [http://www.ccd.com/ Apogee]<nowiki>! Thanks everyone for stopping by at our booth and for all the support our project is getting from the industry!</nowiki>
