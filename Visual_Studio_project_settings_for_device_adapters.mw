{{Note|This page applies to SVN r12232 and later.}}
...
# In my testing, the <code>MyCamera.h</code> and <code>MyCamera.cpp</code> files created in the previous step were blank - although I think they are supposed to contain a skeleton class implementation. In any case, how to actually code device adapters is beyond the scope of this particular page, so lets continue with the project settings.
...
At some point (usually the first time you build or close Visual Studio), you will be prompted to save the solution file (<code>.sln</code>). Saving to the default location (next to the <code>.vcxproj</code> file) usually works well when developing a device adapter. Note that the build products are placed in a <code>build</code> directory in the same directory as the solution file.
...
'''Enable C++ Exceptions''': '''Yes (/EHsc)''' (the default) is recommended. Do not turn on SEH Exceptions; see [http://blogs.msdn.com/b/larryosterman/archive/2004/09/10/228068.aspx this page] for why.
