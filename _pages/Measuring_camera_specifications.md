...
## Visual inspection of fixed pattern noise in dark images
  ...
  - Inspect the resulting data for irregularities. Does the average
    ...
    “Add”, then select “More” -\> “Multi Measure”, make sure
    “Measure all Slices” and “One row per slice” are checked and press
    OK) ?
...
deviation of the pixel intensity values in such a dark image.
  ...
  - To measure the offset, take a single ‘dark’ image from the series
    ...
    of the image or - if the camera has substantial fixed pattern noise
    - a subregion of the image (use the ImageJ command:
  ...
  - To measure the readout noise in digital numbers, take two ‘dark’
    images (you can use ‘Snap’, then ImageJ menu, ‘Image’-\>’Duplicate’,
    then ‘Snap’ the second image). Subtract one image form the other
    (ImageJ menu: Process -\> Image Calculator..., select the two images
    and subtract from each other). Open in a new window and use 32-bit
    (float) result. Calculate the standard deviation of the resulting
    image or a subregion if there are regions with detectable fixed
    pattern noise (ImageJ menu: Analyze-\>Set Measurements.. make sure
    that ‘standard deviation’ is checked, then select
    Analyze-\>Measure). The read-noise (in digital numbers) is the
    measured standard deviation divided by √2. The purpose of
    subtracting the two images is to remove fixed pattern noise. Make a
    note of the read-noise.
...
‘c’ yields: c = DN/σ2(DN) Thus, if imaging conditions can be
established in which the only significant noise source is shot noise,
then the relation between digital number and number of electrons can be
...
￼![media/PhotonTC.gif](media/PhotonTC.gif "media/PhotonTC.gif")
...
calculate the noise after subtracting one image from the other.
  ...
  - To create a Photon Transfer Curve we will take a series of images,
    ...
    ‘[media/CameraLabs.bsh](https://valelab.ucsf.edu/svn/micromanager2/trunk/scripts/media/CameraLabs.bsh)’.
    ...
    [media/CameraAnalysis.bsh](https://valelab.ucsf.edu/svn/micromanager2/trunk/scripts/media/CameraAnalysis.bsh)
...
following reference: James R. Janesick, Photon Transfer, DN -\> λ. SPIE
Press, 2007.
